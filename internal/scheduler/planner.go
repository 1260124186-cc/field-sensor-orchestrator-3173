...
var ErrActiveLease = errors.New("site has active collection lease")
...
func (p *Planner) Schedule(site, actor string, ttl time.Duration) (domain.Cycle, error) {
	for _, cycle := range p.store.CyclesBySite(site) {
		if cycle.LeaseID == "" { continue }
		lease, err := p.store.Lease(cycle.LeaseID)
		if err == nil && lease.ReleasedAt.IsZero() { return domain.Cycle{}, ErrActiveLease }
	}
